...
/* ====== Phân tích hình nêm theo 8 hướng ====== */
const DIRECTIONS=['Bắc','Đông Bắc','Đông','Đông Nam','Nam','Tây Nam','Tây','Tây Bắc'];

function makeWedgePolygon(center,radius,startDeg,endDeg){
  const toRad=d=>d*Math.PI/180;
  const p1={x:center.x+radius*Math.cos(toRad(startDeg)),y:center.y+radius*Math.sin(toRad(startDeg))};
  const p2={x:center.x+radius*Math.cos(toRad(endDeg)),y:center.y+radius*Math.sin(toRad(endDeg))};
  return [center,p1,p2];
}
...
let polygon=[], centerPoint={x:0,y:0}, northRotation=0, sectorCoverage=[];

function analyzeSectors(){
  const total=polygonArea(polygon);
  sectorCoverage=[];
  if(!total) return sectorCoverage;
  const radius=Math.max(...polygon.map(p=>Math.hypot(p.x-centerPoint.x,p.y-centerPoint.y)))*2;
  for(let i=0;i<8;i++){
    const centerDeg=northRotation+i*45;
    const wedge=makeWedgePolygon(centerPoint,radius,centerDeg-22.5,centerDeg+22.5);
    const clipped=sutherlandHodgman(polygon,wedge);
    const area=polygonArea(clipped);
    sectorCoverage.push({direction:DIRECTIONS[i],coverage:area/total,polygon:clipped});
  }
...
function draw(){
  ...
  analyzeSectors();
  sectorCoverage.forEach((s,i)=>{
    const poly=s.polygon;
    if(!poly.length) return;
    ctx.beginPath();
    poly.forEach((p,j)=>{ if(j===0) ctx.moveTo(p.x,p.y); else ctx.lineTo(p.x,p.y); });
    ctx.closePath();
    ctx.fillStyle=`hsla(${i*45},70%,50%,0.3)`;
    ctx.fill();
    let cx=0,cy=0; poly.forEach(p=>{cx+=p.x; cy+=p.y;});
    cx/=poly.length; cy/=poly.length;
    ctx.fillStyle='#000';
    ctx.fillText((s.coverage*100).toFixed(1)+'%',cx,cy);
  });
}
